...
// de forma de cara (Fase 3), el form de 4 taps del barbero (7.7, todavía sin
// UI: ver la nota en `types.ts`) y la base de cortes (`cuts.seed.json`, se
// carga con `fetch` en runtime — cargarlo es responsabilidad de quien llame
// a esta función, no de acá, para mantenerla pura), sale el ranking COMPLETO
// de cortes puntuados (la Fase 5 decide cuántos mostrar y cómo filtrar por
// longitud).
//
// PENDIENTE A PROPÓSITO — "camino en 2-3 cortes" (sección 9): la spec pide
// que un corte que puntúa alto pero necesita más largo del que el cliente
// tiene hoy se muestre como "camino en 2-3 cortes" en vez de ocultarlo. Para
// eso hace falta un dato de LARGO ACTUAL DE PELO DEL CLIENTE que hoy no
// existe en ningún lado del pipeline: ni en `BarberInput` (7.7 solo pide
// textura/densidad/flags/minutos), ni en la clasificación de forma. Agregar
// ese input es una decisión de producto (¿un quinto tap? ¿inferirlo de la
// foto?) que no me corresponde resolver en esta fase — queda explícitamente
// afuera de `recommendCuts` hasta que se defina. Por ahora el ranking no
// distingue "me sirve hoy" de "me serviría si tuviera más largo".
